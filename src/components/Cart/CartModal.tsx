...
export function CartModal() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('nik-electric:open-cart', openCart)

    return () => {
      window.removeEventListener('nik-electric:open-cart', openCart)
    }
  }, [])
        ...
        <SheetContent className="flex flex-col">
          ...
          {!cart || cart?.items?.length === 0 ? (
            <div className="flex flex-col items-center gap-3 pt-8 text-center">
              ...
              <p className="text-center text-2xl font-medium text-primary/75">
                Количката е празна.
              </p>
            </div>
          ) : (
                  ...
                  {cart?.items?.map((item, i) => {
                    ...
                    const price = resolvePriceForTier(
                      (user as typeof user & { priceTier?: 'general' | 'group1' | null })
                        ?.priceTier,
                      {
                        priceGroup1: (product as Product & { priceGroup1?: number | null })
                          .priceGroup1,
                        priceWholesale: (product as Product & { priceWholesale?: number | null })
                          .priceWholesale,
                      },
                    )
                    ...
                    return (
                      <li
                        className="flex w-full flex-col border-b border-black/5 py-4 last:border-b-0"
                        key={i}
                      >
                          ...
                          <div className="flex flex-row items-end justify-between gap-3 sm:h-16 sm:min-w-[88px] sm:flex-col sm:justify-between">
                            ...
                            <div className="ml-auto flex h-8 flex-row items-center rounded-md border border-black/10 bg-white">
                              ...
                              <p className="w-6 text-center">
                                <span className="w-full text-sm text-primary/70">
                                  {item.quantity}
                                </span>
                              </p>
